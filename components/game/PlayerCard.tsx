...
/**
 * A player's card beside the board: who they are, their rating, their clock,
 * and how much of it is left.
 *
 * It WRAPS rather than replaces. `children` is the exact JSX the game page
 * already rendered, clock element and all, so LiveChessClock keeps owning
 * every millisecond of timing and this file contains no clock arithmetic
 * whatsoever.
 *
 * This used to live under components/world and render the plain old row
 * unless a World location was active. That was the wrong condition: nothing
 * about the card needs a location, and gating it on one meant the better card
 * only ever appeared to players who had been to the World. It is the card now,
 * for everyone.
 *
 * On flags: there is no country row, because the app stores no country. The
 * children table has no such column, and the only country the codebase knows
 * is the request-IP one Vercel supplies for Stripe pricing — the paying
 * parent's network location, not the young player's nationality. An invented
      ...
      <div className="flex w-full items-center justify-between gap-3">
        <span className="flex min-w-0 items-center gap-2.5">
          {/* Avatar stand-in. The app has avatar ids on children but the game
              ...
              here — which would put a network call inside a presentational
