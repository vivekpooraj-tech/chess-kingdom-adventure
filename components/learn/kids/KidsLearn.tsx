...
/**
 * Kids ("Enchanted Kingdom") Learn composition — "Learn & Train". Same real
 * content/client islands as every other mode; NextLessonCard and
 * LearningPathPanel keep their own existing internal headings ("Recommended
 * for you" / "Your Learning Path") since those components are not modified
 * here — no duplicate "Try This Next"/"Where You Are" label is added
 * outside them, to avoid a redundant second heading next to their real one.
 * Rows are taller and rounder for touch comfort; all 6 real Learn Chess
 * entries and all 8 real Chess Mind categories render, no XP/levels/stars.
 */
export function KidsLearn({
  learnChess,
  courseLessonIds,
}: {
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      <div>
        <h1 className={TEXT.display}>Learn &amp; Train</h1>
        <p className={`${TEXT.body} mt-2`}>Pick something fun to learn today!</p>
      </div>
      ...
      <section className="w-full flex flex-col gap-3">
        <p className="learn-section-eyebrow font-classic-body text-xs font-semibold uppercase tracking-wide">
          Learn Chess
        </p>
        <div className="flex flex-col gap-2.5">
          {learnChess.map((item) => (
            <ListItemRow key={item.id} href={item.href} className="learn-row min-h-[64px]">
              ...
              <span className="learn-cta text-xl flex-none">→</span>
            </ListItemRow>
          ))}
        </div>
      </section>
