...
function LocationCard({
  ...
  return (
    <article className="world-card w-full rounded-premiumCard overflow-hidden border border-white/10 bg-premium-navy shadow-premiumCard flex flex-col">
      ...
      <div
        className={`world-card-scene relative w-full aspect-[16/9] bg-gradient-to-br ${location.cardGradient}`}
      >
        ...
        <button
          ...
          className="world-favourite-btn absolute top-3 right-3 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-full bg-black/30 hover:bg-black/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70 text-xl"
        >
        ...
        <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between gap-3">
          <div className="min-w-0">
            <p className="world-card-title font-classic-display text-xl text-white drop-shadow">
              <span aria-hidden="true">{location.emoji} </span>
              {location.title}
            </p>
            <p className="world-card-tagline font-classic-body text-xs text-white/80">{location.tagline}</p>
          </div>
          {!preview && (
            <button
              type="button"
              ...
              className="world-lookaround-btn flex-none min-h-[44px] px-3.5 rounded-full border border-white/40 bg-black/30 font-classic-body text-xs font-semibold text-white hover:bg-black/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
            >
              Look around
            </button>
          )}
      ...
      <div className="world-card-body p-4 sm:p-5 flex flex-col gap-3 flex-1">
        <p className={`world-atmosphere ${TEXT.caption} uppercase tracking-wide`}>{location.atmosphere}</p>
        <p className={`world-story ${TEXT.body} flex-1`}>{location.story}</p>

        {/* Only ever real numbers. No entry, no line. */}
        {entry?.playedAt ? (
          <p className={`world-played-line ${TEXT.caption} normal-case`} style={{ color: location.accent }}>
            ✓ {entry.games} {entry.games === 1 ? "game" : "games"} played here
            {entry.wins > 0 && ` · ${entry.wins} ${entry.wins === 1 ? "win" : "wins"}`}
          </p>
        ) : entry ? (
          <p className={`world-played-line ${TEXT.caption} normal-case`}>Visited — no games here yet.</p>
        ) : null}

        <Link
          href={playHereHref(location.id)}
          className="world-play-cta self-start font-classic-body text-sm font-semibold text-premium-midnight bg-premium-gold rounded-full px-5 min-h-[44px] flex items-center active:scale-[0.98] transition-transform duration-100 motion-reduce:transition-none focus:outline-none focus-visible:ring-2 focus-visible:ring-premium-gold/60"
        >
          ♟ Play here →
        </Link>
      </div>
...
export default function WorldPage() {
  ...
  return (
    <TabPageShell maxWidth="wide" contentClassName="world-mode-scope">
      <header className="world-header w-full">
        <p className={`world-eyebrow ${TEXT.meta} text-premium-gold`}>🌍 {WORLD_NAME}</p>
        <h1 className={`world-title ${TEXT.display} mt-1`}>{WORLD_TAGLINE}</h1>
        <p className={`world-lead ${TEXT.body} mt-2`}>
          The rules never change. Everything around them can. Choose a place, and play there.
        </p>
      </header>

      <div
        className="world-grid auto-grid items-stretch w-full"
      ...
      {games > 0 && (
        <section
          ...
          className="world-passport w-full rounded-premiumCard border border-premium-gold/25 bg-premium-navyLight/60 p-5 flex flex-col gap-3"
        >
          <h2 id="passport-heading" className={`world-passport-heading ${TEXT.meta} text-premium-gold`}>
            🗺️ Your World Passport
          </h2>
          <p className={`world-passport-summary ${TEXT.body}`}>
            {games} {games === 1 ? "game" : "games"} played across {played.length}{" "}
            {played.length === 1 ? "location" : "locations"}.
          </p>
          {recentLocation && (
            <p className={`world-passport-recent ${TEXT.caption} normal-case`}>
          ...
          {achievements.length > 0 && (
            <ul className="world-achievements flex flex-wrap gap-2 list-none mt-1">
              {achievements.map((a) => (
                <li
                  key={a.key}
                  title={a.description}
                  className="world-achievement-chip font-classic-body text-xs text-premium-ivory/85 border border-premium-gold/30 bg-white/[0.03] rounded-full px-3 py-1.5"
                >
          ...
          <p className={`world-passport-footnote ${TEXT.caption} normal-case text-premium-ivory/45`}>
